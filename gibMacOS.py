...
class gibMacOS:
    ...
    def macos_to_num(self,macos):
        try:
            ...
            return None
        return macos_parts[1] if len(macos_parts) > 1 else 5+macos_parts[0]
    ...
    def get_dict_for_prods(self, prods, plist_dict = None):
        ...
        for prod in prods:
            ...
            print(
                f' -->{str(len(prod_list) + 1).rjust(3)}. {prodd["title"]} ({prodd["build"]}){" - FULL Install" if self.find_recovery and prodd["installer"] else ""}'
            )
    ...
    def download_prod(self, prod, dmg = False):
        ...
        self.resize()
        name = f'{prod["product"]} - {prod["version"]} {prod["title"]} ({prod["build"]})'.replace(
            ":", ""
        ).strip()
    ...
    def main(self, dmg = False):
        ...
        lines.extend(
            (
                ...
                f'I. Only Print URLs (Currently {"On" if self.print_urls else "Off"})',
            )
        )
    ...
    def get_for_product(self, prod, dmg = False):
        self.u.head(f"Downloading for {prod}")
        print("")
    ...
    def get_for_version(self, vers, dmg = False):
        self.u.head(f"Downloading for {vers}")
        print("")
        ...
        for p in sorted(self.mac_prods, key=lambda x:x['version'], reverse=True):
            ...
            if pv not in [" ", "1.0"] and pv.startswith(v):
                self.download_prod(p, dmg)
                return
...
if __name__ == '__main__':
    ...
    if args.maxos:
        try:
            if version := g.macos_to_num(args.maxos):
                g.current_macos = version
        except:
            pass
    if args.catalog:
        # Set the catalog
